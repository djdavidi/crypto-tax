...
function getAllExchanges() {
    ...
    .then(allExchanges => {
    	let cleanedData = allExchanges.map((exchangeData, index) => {
			return (exchangeData ? cleaningFunctions[index](exchangeData, exchangeNames[index]) : "Api Error")
    	})
...
function cleanKucoinData(data, exchangeName) {
    return JSON.parse(data).data.map(datum => {
        return {
            baseCurrency: datum.coinType.toUpperCase(),
            quoteCurrency: datum.coinTypePair.toUpperCase(),
            price: datum.lastDealPrice,
            exchange: exchangeName,
            name: `${datum.coinType}-${datum.coinTypePair}`.toUpperCase()
        }
    })
}
...
function splitStringIntoTradingPair(tradingPair) {
    ...
    possibleBases.forEach(base => {
        ...
        if (baseIndex > 0) {
            splitPair = [
                tradingPair.slice(base.length, -1),
